...
zoomController.put('/webinar/registrants/status/update', function (request, response) {
    console.log(request.body)
    const { webinarId, status, id, email } = request.body;
    zoomApi.put(`https://api.zoom.us/v2/webinars/${webinarId}/registrants/status`, { action : status , "registrants": [{"id": id,"email": email}]})
        .then(data => response.send(data))
        .catch(err => {
            console.log(err)
            response.status(500).send({
...
zoomController.delete('/webinar/delete', function (request, response) {
    console.log(request.body)
    const { webinarId } = request.body;
    zoomApi.delete(`https://api.zoom.us/v2/webinars/${webinarId}`)
        .then(data => response.send(data))
        .catch(err => {
            console.log(err)
            response.status(500).send({
